...
​		如果您是一名服务器开发者，有没有发现很多服务器框架很庞大，提供了太多的功能，依赖太多的库，构建起来很复杂，甚至要改变自己的使用习惯，已有的系统想用上这个框架需要修改太多的地方，甚至各种冲突。要么封装度不够，构建一个服务器还是有太多的工作要做。要么作者自己做了太多的改造，效果到底怎么样心里没底。如果您有这些顾虑，您可以考虑下asyncio。
...
| 10.user_client       | 用户自定义协议头的解码器客户端示例。我们为解码器提供了一个模板，用户自己定义协议头，该定义多少字段，各个字段什么类型让用户自己决定。 |
| 11.user_server       | 用户自定义协议头的解码器服务器示例。                         |
| 12.conn_test_client  | 连接数测试客户端工具，与6.load_test_server配套，连接建立之后每隔2-3秒钟发送一个ping |
|                      |                                                              |
|                      |                                                              |
|                      |                                                              |
|                      |                                                              |
...
| 7    | 做一个RPC解码器，客户端和服务器示例，已合并到自定义协议头的解码器CodecUserHeader | 20211015 Will 已完成 |
| 8    | 优化定时器接口，通过持有返回值来控制定时器的生命周期         | 20210901 Will 已完成 |
| 9    | 学习下facebook相关开源项目，优化接口                         |                      |
| 10   | 优化定时器接口和使用，解决线程安全问题                       | 20210916 Will 已完成 |
| 11   | 新增用户自定义协议头的解码器CodecUserHeader，使用方法参照10.user_client和11.user_server | 20211015 Will 已完成 |
| 12   | 写一个RPC调用示例                                            |                      |
...
4. 协议头（报文头）该怎么设计？一百个人有一百个想法，所以框架应该把它做成模板，支持用户传入自定义的协议头，一了百了。这个点子来自于Facebook的wangle。
